...
class DiscordProcess:
    ...
    def script_path(self):
        if sys.platform == 'win32':
            ...
            # To get the version number we have to iterate over ~/.config/discordc
            # anary and find the
            # folder with the highest version number
            discord_version = os.path.basename(self.path).replace('-', '')
            config = os.path.expanduser(os.path.join(
            ...
            for subdirectory in os.listdir(config):
                ...
                try:
                    # versions are A.B.C
                    version_info = tuple(int(x)
                                         for x in subdirectory.split('.'))
            ...
            if len(versions_found) == 0:
                raise RuntimeError(
                    'Could not find discord application version under "{}".'.format(config))
...
"""
    ...
    parser.add_argument(
        ...
        required=True,
        metavar="file_or_dir",
        help="Location of the discord installation dir",
    )
    parser.add_argument(
        "--revert",
        action="store_true",
...
def main():
    args = parse_args()
    try:
        discord = DiscordProcess(path=args.path)
    except Exception as e:
        print(str(e))
        return
    ...
    if b"contextBridge.exposeInMainWorld('BeautifulDiscord'," not in preload:
        preload = preload.replace(
            b"process.once('loaded', () => {", load_file_script.encode('utf-8'), 1)
    ...
    # yikes
    to_write = entire_thing[:index] + \
        css_reload_script.encode('utf-8') + entire_thing[index:]
